...
export default function PrivacyPolicy() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const res = await publicClient.get('/legal-pages/public/privacy_policy');
  ...
  return (
    <main className="min-h-screen bg-[#080C1A] text-white">
      ...
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        ...
        <div className="absolute top-1/2 -left-40 w-[500px] h-[500px] rounded-full bg-indigo-600/10 blur-[100px]" />
      ...
      <header className="relative z-20 border-b border-white/5 bg-white/3 backdrop-blur-xl">
        <div className="max-w-5xl mx-auto px-5 py-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-semibold text-violet-400 uppercase tracking-widest">Veerify Platform</p>
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-5 py-14">
        ...
        <div className="mb-14 text-center">
          <div className="inline-flex items-center gap-2 bg-violet-500/10 border border-violet-500/20 rounded-full px-5 py-2 text-sm text-violet-300 font-medium mb-6 backdrop-blur-sm">
            <Shield size={15} />
            Legal &amp; Privacy
          </div>
        ...
        {!loading && !error && (!page || !page.sections?.length) && (
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <p className="text-gray-500">Privacy Policy content hasn't been published yet.</p>
          </div>
        )}
          ...
          <div className="space-y-6">
            {page.sections
              ...
              .map((section, idx) => (
                <div
                  key={section.key}
                  className="group relative bg-white/4 backdrop-blur-md border border-white/8 rounded-2xl p-7 hover:bg-white/6 hover:border-violet-500/20 transition-all duration-300"
                >
                  {/* Glow on hover */}
                  <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-violet-500/0 to-indigo-500/0 group-hover:from-violet-500/5 group-hover:to-indigo-500/5 transition-all duration-500" />

                  <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-4">
                      <span className="w-7 h-7 rounded-lg bg-violet-500/15 border border-violet-500/20 flex items-center justify-center text-xs font-black text-violet-400">
                        {idx + 1}
                      </span>
      ...
      <footer className="relative z-10 border-t border-white/5 mt-20">
        <div className="max-w-5xl mx-auto px-5 py-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-5 text-xs text-gray-600">
            <a href="/privacy-policy" className="hover:text-violet-400 transition-colors font-medium text-violet-400">
              Privacy Policy
            </a>
            <a href="/terms-and-conditions" className="hover:text-violet-400 transition-colors">
              Terms &amp; Conditions
            </a>
            <a href="/refund-cancellation-policy" className="hover:text-violet-400 transition-colors">
              Refund Policy
            </a>
          </div>
        </div>
      ...
      <style>{`
        ...
        .prose-legal a { color: #a78bfa; text-decoration: underline; }
